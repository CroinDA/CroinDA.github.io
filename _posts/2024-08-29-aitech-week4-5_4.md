...
- MLE를 구하는 것이 목표(늘 그래왔듯)
	...
	3. EM-Algorithm 정리
		- E-step, M-step 지속적으로 반복하며 업데이트
			- M-step : 1, 2번
			- E-step : 3번
		1. For q ($\gamma(z_{nk})$는 상수) : $\sum_{n=1}^{N} \gamma(z_{nk}) \cdot f(q_k;x_n) = 0$ → 정리하면 q값 나옴<br><br>
		2. For $\pi$ (($\gamma(z_{nk})$는 상수)) : $\pi_k = \frac{\sum_{n=1}^{N}\gamma(z_{nk})}{N}$ → p값 나옴<br><br>
