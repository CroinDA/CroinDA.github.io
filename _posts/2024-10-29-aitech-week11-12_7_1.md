...
- ==이웃 노드 간 정보== 이용 → 특정 노드를 잘 표현할 수 있는 feature(벡터)를 잘 찾아내는 것
	- 방법: 그래프 / 피처 데이터를 인접 행렬로 변환 → "Naive Approach"        
		![image3](../../images/2024-10-29-aitech-week11-12_7_1/image3.png)           <br><br>
...
- User-item 간 interaction 그래프를 보고, 그 위에서 임베딩을 여러 번 전파하고자 함<br><br>
...
- 3차 연결 (Higher-order Connection):
	...
	- "B가 영화 Y를 봤으니, A도 영화 Y를 좋아할 가능성이 높음"<br><br>
				...
				- $W_1, W_2$ 의 역할
					- 특징 추출/강화 (Feature Transformation)
						...
						- $W_2$
							...
							- 여기에 $W_2$를 곱해주면, 이 값들을 새로운 방식으로 재구성
							  (이전과 다른 Feature 벡터로 바꿔줌 → 원래 벡터의 의미/좌표계/조합을 바꾸어 새로운 방식으로 정보(패턴, 관계)를 표현)
...
- MF vs NGCF
	- MF에 비해 높은 recall값을 나타냄(세 개의 데이터셋으로 실험한 결과)                     
		![image8](../../images/2024-10-29-aitech-week11-12_7_1/image8.png)
	- MF에 비해 빠른 수렴 양상을 보임
		- Embedding Propagation Layer를 통한 representation power 강화
		- MF에 비해 유저-아이템이 Embedding space에서 더 명확히 구분
			- 레이어가 많아질수록 명확                      
		![image9](../../images/2024-10-29-aitech-week11-12_7_1/image9.png)<br><br>
...
- 이러한 이유들로 인해, GCN을 추천시스템에 적용해서 취할 수 있는 장점을 선별하여 적용한 모델이 LightGCN
	...
	- NGCF의 구성 요소들을 제거해본 실험 결과
		...
		![image11](../../images/2024-10-29-aitech-week11-12_7_1/image11.png)         
...
- Model Prediction
	...
	- 특징: 내적을 통한 간단한 점수 계산
	- 개선점: 복잡한 예측 레이어 불필요           
	![image12](../../images/2024-10-29-aitech-week11-12_7_1/image12.png)<br><br>
...
1. Light Graph Convolution
	...
	- 수식
		...
		- 기존 NGCF vs LightGCN
			![image18](../../images/2024-10-29-aitech-week11-12_7_1/image18.png)
...
2. Layer Combination
	...
	- K개의 LGC를 지난 이후, 각 레이어에서 얻은 임베딩 합쳐서 최종 결과 도출
		...
		- $\alpha_k$: LGC 레이어 별 가중치
			...
			- $\alpha_k = 1/(K+1)$
		- 기존 NGCF vs LightGCN
			![image19](../../images/2024-10-29-aitech-week11-12_7_1/image19.png)
	- 3가지 이점
		1. Over-smoothing 방지
...
- LightGCN vs Other competing method
	...
	- 결과 요약
		...
		- 벤치마크 결과 예시(아래 표 참조)
			...
			![image17](../../images/2024-10-29-aitech-week11-12_7_1/image17.png)
- LightGCN 모델이 갖는 의의
