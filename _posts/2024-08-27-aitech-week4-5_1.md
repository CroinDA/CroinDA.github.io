...
- MLE(Maximum Likelihood Estimation, 최대우도법 / 최대가능도법, $\theta^*$)
	- 각 관측값의 총 가능도가 최대가 되도록 하는 확률분포를 찾는 것
...
- Prior(신뢰도, 사전확률)
	- 모수에 대한 사전 지식이나 신뢰도를 확률로 표현한 것
		- 전통적인 통계에서는, 확률을 "<mark style="background: #FFF3A3A6;">몇 번이 등장하는가</mark>"로 보는 Frequentism의 관점으로 봄
	...
	- 베이즈 정리(베이지안 통계) : $p(\theta \vert x^{(1)}, \dots, x^{(m)}) = \frac{p(x^{(1)}, \dots, x^{(m)} \vert \theta) p(\theta)}{p(x^{(1)}, \dots, x^{(m)})}$
		- Posterior = Likelihood * Prior / Evidence
			- Posterior : 가설에 대한 확률(Prior)이 evidence(새로운 정보)를 관측하여 갱신된 후, 해당 가설에 대한 신뢰도 (데이터를 given으로 줄 때, $\theta$에 대한 prior가 결합하여 $\theta^*$ 추정)
			- 분자 - Likelihood(주어진 데이터에 기반하여, 가설에 가장 부합하는 $\theta$ 추정) * prior(가설에 대한 신뢰도)
			- 분모 - 가설을 갱신시킬 새로운 정보 / 데이터(Evidence)
