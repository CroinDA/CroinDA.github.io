...
## 1. Retrieval
### 개요
- 고차원 데이터를 벡터 공간에 매핑해 의미적 유사성을 계산하는 기술
- 단어들의 의미가 유사하면 해당 단어들에 대한 임베딩 벡터가 벡터 공간 상에서 가깝다는 점을 활용
...
- 임베딩 방법에 따라, 다른 modality를 가진 데이터를 검색할 수 있음
	- 텍스트로 이미지 검색<br><br>
...
- Facial Recognition 내 Retrieval 과정
	...
	3. 데이터베이스 매칭(Retrieval)
		...
		- Retrieval 활용의 이유
			- 모든 개인을 클래스로 두고 학습하는 것은 무리
			- DB 내 사진과의 거리 비교를 통해 가까운 거리에 위치한 사진에 있는 얼굴을 테스트하는 사람의 얼굴이라 판단
		...
		- 유사도 계산 방법
			...
			- 임계값 설정
	4. 신뢰도 평가 및 결과 출력
		- "두 얼굴이 동일인일 확률"로 해석, 이를 기반으로 신뢰도 산출
			1. 분포 기반 확률 계산
			...
			2. 단순 스케일링
				- 코사인 유사도(범위 -1~1)를 신뢰도(범위 0~1)로 단순 선형 변환
					- $confidence = \frac{cosine_{similarity} + 1}{2}$
...
3. Cosine Similarity
	...
	- $d(p,q) = 1-\frac{p \cdot q}{|p||q|}$<br><br>
...
- 개요
	...
	- ex) 아래와 같이 입력된 피처 $x$(아래 그림에서, 파란 점)가 있다면
		...
		- Decision Boundary: 입력 피처 $x$에 대해, $x$가 가중치 벡터 $W_1$ 및 $W_2$와 이루는 각도가 같아지는 경계선
			...
			![image3](스크린샷%202025-06-25%20오후%208.45.11.png)
- 학습 방법
	...
	- Angular margin
		...
		- ex) 만일 x가 클래스 1이라면, $logit_1$ 계산 시 각도 $\theta_1$에 마진 $m$을 더해줌
			- 마진 m을 더해도 아래 식이 성립하여서, x가 클래스 1으로 분류될 수 있도록 해줌
			- $logit_1 = cos(\theta_1+m) > logit_2 = cos(\theta_2)$
				![image4](스크린샷%202025-06-25%20오후%209.15.12.png)
		- 특징
			1. Angular margin 원리에 따라 학습하면, $\theta_1$이 작아지는 방향으로 학습됨
				- 이유 → x가 기존 Decision boundary에 가까운 곳에 있다면, m을 더하는 순간 클래스 2로 구분될 수 있기 때문
					![image5](스크린샷%202025-06-25%20오후%209.18.54.png)
				- 따라서, x는 최소한 decision boundary로부터 마진 m 이상 떨어진 곳에 위치해야 한다
					- 아래 예시같은 경우, 당연히 반대편 클래스 2의 경우에 대해서도 동일
					![image6](스크린샷%202025-06-25%20오후%209.21.48.png)
			2. 위 1번과 같은 특징으로, 임베딩들은 최소한 decision boundary에서 $m$만큼 떨어진 곳에 분포함
			3. 그렇기 때문에, 결과적으로 임베딩들은 가중치 벡터(아래 예시에서는 $W_1$, $W_2$)를 중심으로 분포하게 됨
				![image7](스크린샷%202025-06-25%20오후%209.27.10.png)
			4. 마진 m이 너무 커질 경우, 학습이 잘 이루어지지 못함
				...
				- 마진이 클수록 같은 클래스 끼리 모이는 힘이 강해지지만, <mark style="background: #FFF3A3A6;">너무 커지면 물리적으로 학습이 불가능한 상태</mark>에 도달하게 됨
					![image8](스크린샷%202025-06-25%20오후%209.32.44.png)
		- 학습 결과 비교(Normal Softmax vs Arcface)
			...
			- Arcface(우, Angular margin): classification을 수행한 결과가 decision boundary에서 <mark style="background: #FFF3A3A6;">멀리</mark> 떨어져 <mark style="background: #FFF3A3A6;">집중</mark>되어 있음 
				![image9](스크린샷%202025-06-25%20오후%209.38.35.png)<br><br>

### CLIP(Contrastive Language-Image Pre-training)
- 이미지-텍스트 간 연관성을 학습하는 멀티모달 모델 / OpenAI 개발
- 핵심 아이디어
	- 데이터
		- 대규모의 이미지-텍스트 쌍의 데이터를 활용(4억 쌍의 pair data 활용)
		- 이미지-텍스트 데이터를 동일한 임베딩 공간에 매핑하는 것
	- 학습방법
		- Contrastive Learning → 모 이미지의 임베딩과 그 이미지에 해당하는 텍스트의 임베딩은 가까워지도록 학습함(그렇지 않은 텍스트의 임베딩은 멀어지도록)
...
- CLIP의 구조 및 동작 원리
	...
	- 학습 방법: <u>Contrastive Learning</u>
		- 이미지 별로, 해당 이미지(보라색 열)를 설명하는 텍스트(초록색 행)는 임베딩 공간에서 벡터를 가까이 위치시킴 → 해당 "이미지 벡터-텍스트 벡터"의 내적값(파란색)을 크게 해줌
			- 나머지 서로 관련없는 이미지-텍스트 벡터의 내적값은 작게 해줌
				![image10](스크린샷%202025-06-26%20오전%2012.52.03.png)
		...
		- 이렇게 대형 데이터를 학습시키고 나면, <mark style="background: #FFF3A3A6;">언어 능력을 갖춘 이미지 모델</mark>을 만들 수 있음
			- 학습에서 본 적이 없는 클래스의 이미지도 이미지 분류가 가능해짐
			  (by. 텍스트 설명을 활용)
			- 일반적인 이미지 분류 모델이 분류하는 방법 대신, <mark style="background: #FFF3A3A6;">텍스트를 직접 입력</mark>하여 이미지 임베딩과 내적값이 가장 큰 클래스를 선택하는 알고리즘
			  (텍스트는 전체 문장이 하나의 임베딩 벡터여야 함에 유의)
			- ex) 입력된 강아지 사진의 임베딩 벡터는 "A photo of a dog"라는 문장에 대한 임베딩 벡터와의 내적값이 가장 큼 → 해당 이미지는 dog로 분류
				![image11](스크린샷%202025-06-26%20오전%201.14.03.png)
	...
	- 활용: Open Vocabulary Object Detection
		- 사전 정의된 카테고리의 한계를 넘어, 학습 단계에서 보지 못한 새로운 객체를 탐지하고 분류하는 CV 기술
...
- DINO 학습 과정
	...
	3. 예측과 정렬
		...
		- 같은 이미지에서 나온 서로 다른 views(예: 전체와 부분)의 임베딩이 서로 비슷해지도록 Student를 학습시킵니다.
			...
			![image14](스크린샷%202025-07-07%20오후%206.10.31.png)
