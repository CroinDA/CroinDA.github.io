...
- Variational Inference → 생성 모델을 학습시키는 대표적인 방법론
	...
	- 생성모델: “$x$(이미지) or $(x, y)$(이미지, 레이블)는 어떤 분포에서 sampling 되었을까?"
		...
		- <mark style="background: #FFF3A3A6;">Maximum likelihood</mark>
			- 표현
				- $\theta \leftarrow \text{argmax}_{\theta} \frac{1}{N} \sum_i \log p_{\theta}(x_i)$
				- $\theta \leftarrow \text{argmax}_{\theta} \frac{1}{N} \sum_i \log \int p_{\theta}(x_i, z)$ 
					...
					- $z$ : 이미 관측된 $x$ 제외한 모든 데이터가 어느 클러스터에서 생성되었는지
						...
						![image2](스크린샷%202025-04-18%20오후%2010.19.05.png)
			- 의미
				- 데이터(검은색 점)를 가장 잘 만들어내는 $\theta$ 추정
				  (그래프 파란 선, Log Likelihood 최대화)
					![image1](스크린샷%202025-04-18%20오후%2010.08.43.png)
		- Expected log-likelihood
			- 표현
				- $\theta \leftarrow \text{argmax}_{\theta} \frac{1}{N} \sum_i E_{z \sim p(z \vert x_i)}\log p_{\theta}(x_i, z)$
...
- Variational Inference의 필요성
	...
	2. $E_{z \sim p(z|x_i)}$ → posterior 확률분포는 어떻게 구함?<br><br>
...
- 그렇다면, KL을 줄이기 위해 q를 어느 곳으로 옮겨야 할까…?
	![image5](../../images/스크린샷%202025-04-22%20오후%2010.33.04.png)
	- 좌측 - P에 가중치
		- 어떻게든 Q가 P에 걸쳐야
	- 우측 - Q에 가중치
...
$$
    ...
    - $\text{KL} \geq 0$  이므로 → $\log p(x_i) \geq L_i(p, q_i)$<br><br>
...
- 이 식에서 확인해보면 - $\log p(x_i) = L_i(p, q_i) + \mathrm{KL}(q_i(z) \Vert p(z \vert x_i))$
	...
	3. KL-Divergence가 0이 되기 위해서는, posterior($p(z \vert x_i)$)와 $q_i(z)$가 최대한 유사해져야 함<br><br>

4. 결론
- 가장 처음으로 돌아가서, 변분추론을 해야 하는 이유는 아래 2가지
	1. $z$는 왜 posterior에서 sampling하는가?
	2. $E_{z \sim p(z|x_i)}$ → posterior 확률분포는 어떻게 구함?
- 이에 대한 답변을 정리해보면, 다음과 같음
	- $KL(q_i(z)||p(z|x_i)$) → posterior 근사화는 매우 중요한 과정
		- 궁극의 목표
...
- VI를 통해 이루고자 하는 목표 : 확률분포 $q$를 posterior에 근사
	- But, 모든 확률분포가 하나의 파라미터만 갖는 것은 아님
		- 아래와 같은 상황일 때, 확률분포는 $\alpha, \theta, \eta, \beta, z$ 5개 파라미터를 지님
			![image6](스크린샷%202025-04-23%20오후%2012.09.54.png)
